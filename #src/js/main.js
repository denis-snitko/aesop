...
jQuery(function ($) {
  ...
  $('.s-checkout-form__check-label').on('click', function (el) {

    $('.s-checkout-form__check-label').removeClass('s-checkout-form__check-label--toggleBg')

    if ($(this).closest('input').attr('checked') !== 'checked') {
      $(this).addClass('s-checkout-form__check-label--toggleBg')
    }

    if (el.target.getAttribute('value') === 'courier') {
      $('.show-courier').show(150)
      $('.show-post').hide(150)
    } else if (el.target.getAttribute('value') === 'post') {
      $('.show-courier').hide(150)
      $('.show-post').show(150)
    }
